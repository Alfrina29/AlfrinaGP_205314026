...
public abstract class Epidemic_IQLCC extends ActiveRouter implements CVDetectionEngine, QVDetectionEngine {
     ...
     */ // nilai kemacetan - rasio penurunan dan replikasi, dihitung dengan persamaan EWMA
    private double CV = 0;
     ...
     */
    private static final int DC = 2; // ini nnti ga dipake
    /**
    ...
    protected void initQL() { //inisialisasi
     ...
     */ //nnti di sni aku ngubah cv nya di ganti sama buffer occ nya , terus miiih action yang bakal di pilih sesuai dnegan statenya
    @Override
    public void update() {
        super.update();
    ...
    protected void updateState(int newstate) {
        ...
        // double state = staterequirement(this.CV, this.countcv());
        this.QL.UpdateState(oldstate, actionChosen, reward, newstate);
        double state = this.countcv();
        ...
        if (state > CTH) {
            System.out.println("Nilai CV = " + state);
            System.out.println("Congested");
            System.out.println(newestAction);
            System.out.println(reward);
            System.out.println("");
        } else if (state > NCTH && state < CTH) {
            System.out.println("Nilai CV  = " + state);
            System.out.println("Partial C OR Decrese");
            System.out.println(newestAction);
            System.out.println(reward);
            System.out.println("");
        } else {
    ...
    public void actionSelectionController(int action) { //untuk memilih action mana yang akan diambil pada state yang sedang dialami oleh buffer
        switch (action) {
            ...
            case 0:
                this.dropbasedonhighestrate(); //ini di pake
                break;
            case 1:
                this.dropbasedonhighestnrofreps(); //ini dipake
                break;
            case 2:
                this.dropbasedonoldestTTL(); //ini dipake
                break;
            case 3:
                this.dropbasedonoldestReceivingTime();// ini dipake
                break;
            case 4:
     ...
     */
    protected int staterequirement(double oldcv, double newcv) { //menentukan state dari buffer
        if (newcv >= CTH) {
 ...
 /* if dropByOldestReceivingTime is true, message are deleted by the oldest
    ...
    private void increasemessagegenerationperiod() { //ini nnti dihapus
        this.msggenerationinterval *= k;

    }

    private void decreasemessagegenerationperiod() { //ini nnti dihapus
        this.msggenerationinterval /= k;

    }

    private void decreasingnrofreps() { //ini nnti dihapus
        this.msglimit = (int) Math.ceil(this.msglimit * md);

    }

    private void increasingnrofreps() { //ini nnti dihapus
