...
public abstract class Epidemic_IQLCC_Cia_abal extends ActiveRouter implements QVDetectionEngine, BufferOccDetectionEngine {
     ...
     */
    public Epidemic_IQLCC_Cia_abal(Settings s) {
        super(s);
     ...
     */
    protected Epidemic_IQLCC_Cia_abal(Epidemic_IQLCC_Cia_abal r) {
        super(r);
    ...
    public void changedConnection(Connection con) {
        if (con.isUp()) {
            ...
            Epidemic_IQLCC_Cia_abal peerRouter = (Epidemic_IQLCC_Cia_abal) otherHost.getRouter(); //deklarasi objek
            ...
            //Epidemic_IQLCC_Cia1 peerRouter = (Epidemic_IQLCC_Cia1) otherHost.getRouter();
            //       exchangemsginformation(); //Ini nanti dia bertukar informas
    ...
    public void update() {
        ...
        if ((SimClock.getTime() - LastUpdateTimeofState) >= stateUpdateInterval) {
            ...
            BufferandTime nilaiBuffer = new BufferandTime(newBufferOcc, SimClock.getTime());
            bufferandtime.add(nilaiBuffer);

            int newState = staterequirement(this.bufferOcc, newBufferOcc);
            this.updateState(newState);
            actionChosen = this.QL.GetAction(newState);
...
//        }
    ...
    protected void exchangemsginformation() {
        ...
        for (Connection con : getConnections()) {
            ...
            Epidemic_IQLCC_Cia_abal other = (Epidemic_IQLCC_Cia_abal) peer.getRouter();
...
//            }
//        }
//        if (newstate == PC) { //buat lempar koin untuk probabiiti pemilihan actionnya di coba di sini dlu ye 
...
//        }
    ...
    protected int staterequirement(double oldBufferocc, double newBufferocc) { //menentukan state dari buffer
        if (newBufferocc >= CTH) {
            return C;
        } else if (oldBufferocc > NCTH && newBufferocc < CTH) {
            return PC;
        } else {
            return NC;
        }
    ...
    protected boolean makeRoomForMessage(int size) { //nnti di perbaiki di sini ya cia untuk yang forced drop dan random drop nya
        ...
        for (Message m : message) {
            if (bufferFree >= size) {
                break;//berhenti jika ruang buffer sudah cukup spacenya
//                        deleteMessage(m.getId(), true);
//                        nrofdrops++;
//                        bufferFree += m.getSize();  
            }
